...
!           ---------------
!
            real(kind=RP), parameter :: angle = PI / 6.0_RP
            real(kind=RP), parameter :: x0 = 1.0_RP / 6.0_RP
            ...
            do eID = 1, mesh % no_of_elements
               ...
               do k = 0, Nz;  do j = 0, Ny;  do i = 0, Nx
                  ...
                  xwave = y * tan(angle) + x0

                  if (x <= xwave) then
                     rho = 8.0_RP
                     u = 7.144709581221619_RP
                     v = -4.125_RP
                     p = 116.5_RP !/ gammaM2
                  else
                     rho = 1.4_RP
                  ...
                  Q(3) = rho * v
                  Q(4) = 0.0_RP
                  Q(5) = p / (gamma - 1._RP) + 0.5_RP * rho * (u**2 + v**2)
...
!
            real(kind=RP), parameter :: angle = PI / 6.0_RP
            real(kind=RP), parameter :: x0 = 1.0_RP / 6.0_RP
            real(kind=RP), parameter :: vwave = 10.0_RP
            ...
            associate(gammaM2 => dimensionless_ % gammaM2, gamma => thermodynamics_ % gamma)
               ...
               xwave = x(2) * tan(angle) + x0 + vwave / cos(angle) * t
